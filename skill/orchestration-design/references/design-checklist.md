...
A graph is many loops; a weak verifier now burns tokens in parallel. Cap it three ways: attempt counters on loop-back edges, `recursion_limit` on invoke, and a budget field in state that routers check.
