...
description: Use when planning or fixing multi-step work, software or process: decide how much orchestration it needs, design it, build it on any runtime or none, and verify the build matches the design. SOFTWARE: multi-agent system, agent workflow, pipeline, batch job, ETL, RAG ingestion, CI/CD, LangGraph, StateGraph, fan-out/fan-in, supervisor/worker, reviewer/verifier agents, splitting one script into several. PROCESS: audit, review, migration, research project, compliance pass, manuscript — long model-run work across sessions where coverage matters. DESIGNING, symptoms: "my pipeline is a mess", "one bad step kills the whole run", "it re-runs everything on failure", "I keep losing track of where I am", "I don't know what I've already covered", "my report looked finished but wasn't", "how do I structure this so I stop missing things". STUCK MID-BUILD: "stuck in a loop", "failing tests repeatedly", "it keeps looping forever", "costs exploded", "retries are burning the budget".
---
...
**3b. Implement.** Read the chosen target file. **If the user arrives already building and stuck — looping, repeatedly failing tests, burning budget on retries — do not design anything. Route straight to `modules/` and skip phases 0–2 entirely**; a new architecture cannot fix a wrong premise, and proposing one wastes the turn they needed. Come back here only if the intervention shows the design itself is wrong. Four rules hold on every runtime:
...
2. **Every bound is live.** Force the condition each one guards — a reviewer that never passes, a source that always fails, a counter that never satisfies — and assert the run stops at the cap instead of looping forever. **Where two bounds guard one loop the tighter masks the other**, so assert each one *separately* as the thing that stops the run, with the rest slackened — a spend field that silently replaces instead of accumulating passes every test whose attempt cap fires first. At level 1 this is your retry and timeout caps; they still need proving.
