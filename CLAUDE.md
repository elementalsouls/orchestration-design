...
- `skill/orchestration-design/` — skill source:
  ...
  - Phase 2.5 is a **visual review gate and a hard stop**: hand over an ASCII sketch, the Mermaid block, and the tables; **offer** a pre-filled mermaid.live link (`tools/mermaid_link.py`) rather than emitting one — an unrequested opaque `#pako:` URL to a third party reads as exfiltration, and a routing test had one flagged by security tooling; never mint one autonomously. Ask one specific question ("which node would you delete?"); then **end the turn**. The ASCII is a preview and is never asserted against. No implementation in the same turn. If running autonomously, label the result DESIGN NOT REVIEWED rather than treating absence as approval.
