...
>
> A skill that tells you **how much orchestration your work actually needs** — usually less than you think — designs it before you write a line, and drags your agent out of an execution loop when you're already stuck in one.
>
> 12 markdown files. No runtime, no dependencies, nothing to import.

**Skip to what you need:**
[I already built it and it's bad](#a-you-already-built-it-and-its-bad) ·
[I'm about to build something](#the-ladder) ·
[I'm stuck in a loop *right now*](#when-youre-already-building-and-stuck) ·
[I'm here to check your homework](#the-research)

---

## How you got here

You added an agent. That agent needed a reviewer. The reviewer needed context, so you added a router. The router needed to know when to stop, so you added a supervisor.

It is now 3am. The same test has failed four times. Your token bill has tripled. The bug is exactly where it was six hours ago.

Somewhere in there is a question nobody says out loud: **did any of this need to exist?**

The honest answer is unpopular, which is roughly why nobody publishes it. There is no engagement in a post titled *"You probably need one agent."* So here it is, with citations.

---

## Watch it say no

**You:** *"Build me a multi-agent system that fetches an article, chunks it, summarizes each chunk, reviews the summary, and formats it as markdown."*

**It:**

> Fetching is a tool call. Chunking is a `for` loop. Formatting is a template. That's three functions, not three agents.
>
> And your "reviewer" is the same model grading its own homework, so it will approve almost anything.
>
...
That's the whole product: an advisor with the nerve to interrupt you, and a named reason every time it does.
...
Then just say *"my pipeline is a mess"* or *"I'm stuck in a loop."* You never invoke it by name — it reads the room and loads itself.

Runs on **Claude Code** and **Hermes**. `./build.sh` installs for Claude Code; on Hermes, point it at `skill/orchestration-design/`. It's markdown, so it travels.
...
| *"Everyone's using multi-agent — should we?"* | Usually not. Sometimes yes. The skill names the trigger either way. |
| *"We're fanning out over 10,000 items and it's fragile."* | That trigger is real. Level 5 is right; it just needs bounds and one writer. |
...
Sixty seconds, no download, and it's a little rude. **List every field in your shared state. For each one, count the nodes that write it.**
...
If that turned something up, the rest of this is worth your time. If it didn't — you may genuinely not need this, and that's a perfectly good outcome. Go enjoy your afternoon.

---

## What it hands you

Not advice. A diagram you can argue with.

This is the real output for *"3,000 customer reviews a week across 200 restaurant locations — find out what's going wrong at each one"*:
...
**The colours are information, not decoration.** Grey is deterministic code — free. Blue is exactly one model call — one bill. Red is the exhaustion terminal: bounds hit, work ships *marked* rather than silently. There's a fifth, amber, for an agent node that loops with tools until it decides it's done — **unbounded by default**. Its absence here tells you nothing in this design can run away.
...
### A. You already built it, and it's bad

**Start here.** This is the common case and the honest front door.

The skill reconstructs the design your code *actually* implements — nodes, edges, state ownership, bounds — then diffs it against what a good design would be.
...
The hard stop at Phase 2.5 is the part that earns its keep. You get an ASCII sketch, a Mermaid block, an *offer* of a pre-filled [mermaid.live](https://mermaid.live) edit link — offered rather than pasted, because an unrequested opaque URL to a third-party site is the wrong default for a proprietary architecture — and one **specific** question: *"which node would you delete?"*

Open approval questions get "looks fine." Specific ones get real answers. People cut more than they add once they can see the shape.
...
**Landing on level 1 or 2 is a successful use of this skill**, and the most common correct outcome. These are not tiers you graduate into.

**Landing on level 5 or 6 is equally successful** — just rarer. When the trigger is literally true, the skill doesn't talk you out of it: it designs the fan-out, puts an append reducer on every field the branches write, asserts `len(results) == len(items) - len(errors)` so silent loss can't hide, and hands you a runnable implementation on your framework. The gate exists to make the climb *earned*, not to cap you at level 3.

---

## When you're already building, and stuck

The ladder decides *what to build*. It does nothing for the other failure: the design is right, the level is right, and execution is going in circles anyway.

Re-architecting will not save you here. **A new topology cannot repair a wrong premise** — and climbing a level to escape a loop just spreads the same guessing across more nodes and more spend.

So there's a second layer. `modules/` holds self-contained protocols you invoke *instead of* designing. Arrive already stuck and you skip phases 0–2 entirely.

| Module | Fires | What it forces |
|---|---|---|
| **`context-auditor`** | **Before** a loop | Every count, path, version and name in an always-loaded file is a claim the world can invalidate — and from the inside, a stale fact and a true fact look identical. Verifies them, and cuts rules whose reason nobody can state. |
| **`rubber-duck-verifier`** | **During** one | Stop writing code. Text-only tear-down — goal, the failure quoted exactly, what each attempt *disproved*, what you still don't know — before any further edit. |
| **`adversarial-reviewer`** | **After** work exists | Reviews from a separate clean context, given the artifact and the requirement but never the reasoning that produced it. Its job is to break the code. "Looks good" without saying what was tried is not a review. |
...
Everyone quotes the 90.2%. Almost nobody quotes the next sentence.

This skill isn't a set of opinions about architecture. It's a reading of the 2025–2026 literature turned into a decision procedure, and every default in it traces to one of these.
...
**Where they converge:** most people building agent systems right now are paying multi-agent prices for single-agent quality.

Two rules fall out, and they hold at every level of the ladder:
...
`references/evidence.md` carries the full reading, **dated**, and separates controlled experiments from single-company production reports — so you can see the shelf life and weigh each claim yourself. If the models get dramatically better at coordinating, the loop-first default weakens, and the file says so out loud.
...
That file also carries the pattern that generalises furthest: **generate the "not covered" section from your ledger rather than from memory — every time, including when it's empty.** A deliverable that lists what was found and stays silent on what was never examined implies coverage it didn't achieve. That's the default failure of every report, review and summary written from recall.
...
The reviewer caught a cross-account data leak filed as P2 and raised it to P0. That's the entire argument for level 3, in one line.
...
**A loop that did nothing, enthusiastically.** A design added a cycle so findings could feed new work back into the pipeline. Sound premise — except no node *inside* the cycle wrote the state the re-entry point read. One grep proved it:
...
Rounds 2 and 3 did no work whatsoever, and the run declared victory. **~120 lines of loop, bounds and dryness machinery deleted**; the implementation went from ~230 lines to ~110 and did strictly more.

**A bound that was decorative.** A spend budget incremented at the live API call — so in the project's own mock mode it never incremented, the cap never fired, and the bound was **untested by every test that existed**. Caught by Phase 4's *"force the condition each bound guards."* Moving the counter to the dispatch point made it real.
...
## How it was tested — by agents who'd never heard of it

You cannot evaluate a document you wrote from memory. You'll read what you meant, not what's on the page.

So half the runs were **cold-context**: a fresh agent given only the user's request and the installed skill file. No knowledge of this project, no access to the repo, no idea what answer was wanted.

**Eight end-to-end runs on the skill — four by the author, four cold** — plus a ninth cold run against the [skill-design standard](docs/skill-design-standard.md) itself, which had a fresh agent author a brand-new skill from that document alone.
...
Every defect below came from *using* the skill, not reading it. Each traceable to a commit:
...
**Twenty-five defects, twenty-four of them from a cold run or an audit against one.** The number keeps climbing because the method keeps working, not because the document keeps rotting.
...
```bash
python3 run_checks.py            # every check, one exit code
python3 run_checks.py --setup    # create .venv and install langgraph first
```

Four of the five implementations need `langgraph`; `--setup` installs it into a local `.venv`. Without it those checks report `NO-DEP` and fail loudly rather than skipping green — use `--allow-skip` if you want them tolerated. None of this is needed to *use* the skill; it's how you check the examples still work after an edit.

---

## Why you might actually keep this
...
### Steal the linter, ignore the rest
...
**The most useful thing you can send is a counterexample.** This skill makes a falsifiable claim — that a named trigger, and only a named trigger, justifies each climb. If you have work where the ladder gave the wrong answer, that's worth more than a typo fix. Open a [counterexample issue](.github/ISSUE_TEMPLATE/counterexample.md) with the task, the level it picked, and what actually turned out to be right.

**Being wrong yourself still counts** — *"it said level 1, I built level 3, and level 1 would have been fine"* tells us a trigger reads weaker than it is, and nobody files that one unless invited to. Consider yourself invited.
...
Before opening a PR, run `python3 run_checks.py` — one command, one exit code — and read `CLAUDE.md`, which carries the conventions this repo holds itself to (including the 160-line budget on `SKILL.md`, which is measured, not aspirational).
